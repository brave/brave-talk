...
const useBraveRequestAdsEnabledApi: boolean =
  !(window.chrome == null) && !(window.chrome.braveRequestAdsEnabled == null)
...
const userIsSubscribed = async (): Promise<boolean> => {
  ...
  const subscriptionCheck = import('./subscriptions')
    .then(async (s) => await s.checkSubscribedUsingSDK())
    .finally(() => clearTimeout(timer))

  return await Promise.race([timeout, subscriptionCheck])
}
...
const isOptedInToAds = (): boolean => {
  ...
  if ((button != null) && button.style.display !== 'none' && button.style.display !== '') { return true }
...
const calcBrowserCapabilities = async (): Promise<BrowserProperties> => {
  ...
  const androidP = !(userAgent.match(/Android/i) == null)
